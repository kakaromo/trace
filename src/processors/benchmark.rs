use crate::parsers::{BenchmarkParser, LogLineType, parse_log_file_high_perf};
use crate::utils::{IterationOutputManager, read_to_string_auto};
use crate::TraceType;
...
pub fn parse_benchmark_log(
    ...
    // 전체 parquet 파일 저장
    let base_parquet_path = format!("{}trace_output.parquet", output_prefix);
    if !processed_all_ufs.is_empty() || !processed_all_block.is_empty() || !processed_all_ufscustom.is_empty() {
        log!("Saving combined parquet file: {}", base_parquet_path);
        if let Err(e) = save_to_parquet(&processed_all_ufs, &processed_all_block, &processed_all_ufscustom, &base_parquet_path, 50000) {
            log!("Warning: Failed to save combined parquet: {}", e);
        }
    }
    
    // result.log 생성
    let result_log_path = format!("{}result.log", output_prefix);
    save_result_log(&result_log_path, &processed_all_ufs, &processed_all_block, &processed_all_ufscustom)?;
    ...
    for line in log_content.lines() {
        ...
        match line_type {
            LogLineType::FioResult { iteration, test_type, bandwidth } => {
                log!("Found FIO result - Iteration {}: {} = {} MiB/s", iteration, test_type, bandwidth);
            ...
            LogLineType::TioTestResult { iteration, test_type, bandwidth } => {
                log!("Found TIOtest result - Iteration {}: {} = {} MB/s", iteration, test_type, bandwidth);
            ...
            LogLineType::IOzoneResult { iteration, test_type, bandwidth } => {
                log!("Found IOzone result - Iteration {}: {} = {} MB/s", iteration, test_type, bandwidth);
    ...
    for (iteration, traces) in iteration_traces.iter() {
        ...
        if !traces.block_traces.is_empty() {
            log!("  Parsing {} Block trace lines...", traces.block_traces.len());
            process_trace_type(
                &output_manager,
                *iteration,
                &traces.block_traces,
        ...
        if !traces.ufscustom_traces.is_empty() {
            log!("  Parsing {} UFSCustom trace lines...", traces.ufscustom_traces.len());
            process_trace_type(
                &output_manager,
                *iteration,
...
fn process_trace_type(
    ...
    // 임시 파일에 trace 라인 저장
    let temp_trace_file = format!("/tmp/trace_temp_{}_{}.log", iteration, filename);
    {
            ...
            match trace_type {
                TraceType::UFS => {
                    if !processed_ufs.is_empty() {
                        if let Err(e) = save_to_parquet(&processed_ufs, &vec![], &vec![], parquet_path.to_str().unwrap(), 50000) {
                ...
                TraceType::Block => {
                    if !processed_block.is_empty() {
                        if let Err(e) = save_to_parquet(&vec![], &processed_block, &vec![], parquet_path.to_str().unwrap(), 50000) {
                ...
                TraceType::UFSCUSTOM => {
                    if !processed_ufscustom.is_empty() {
                        if let Err(e) = save_to_parquet(&vec![], &vec![], &processed_ufscustom, parquet_path.to_str().unwrap(), 50000) {
...
fn save_benchmark_results_csv(
    ...
    for (iteration, tool) in sorted_keys {
        ...
        let write_str = write.map(|w| format!("{:.2}", w)).unwrap_or_else(|| "".to_string());
        let read_str = read.map(|r| format!("{:.2}", r)).unwrap_or_else(|| "".to_string());
...
fn save_benchmark_results_json(
    ...
    let max_iteration = sorted_results.iter().map(|(i, _, _, _)| *i).max().unwrap_or(0);
    
    for iteration in 1..=max_iteration {
        for (iter, tool, test_type, bandwidth) in &sorted_results {
            if *iter == iteration {
                let entry = tool_data.entry(tool.clone()).or_insert((Vec::new(), Vec::new()));
    ...
    for (tool_idx, tool) in tool_keys.iter().enumerate() {
        ...
        let comma = if tool_idx < tool_keys.len() - 1 { "," } else { "" };
...
fn save_result_log(
    ...
    if !ufs_data.is_empty() {
        ...
        let read_count = ufs_data.iter().filter(|u| u.opcode.contains("READ")).count();
        let write_count = ufs_data.iter().filter(|u| u.opcode.contains("WRITE")).count();
        let sync_count = ufs_data.iter().filter(|u| u.opcode.contains("SYNC")).count();
        ...
        if let Some(first) = ufs_data.first() {
            if let Some(last) = ufs_data.last() {
                writeln!(writer, "Time range: {:.3} - {:.3} ms", first.time, last.time)?;
            }
        }
        writeln!(writer)?;
    }
    ...
    if !block_data.is_empty() {
        ...
        let read_count = block_data.iter().filter(|b| b.io_type.contains("R")).count();
        let write_count = block_data.iter().filter(|b| b.io_type.contains("W")).count();
    ...
    if !ufscustom_data.is_empty() {
        ...
        if let Some(first) = ufscustom_data.first() {
            if let Some(last) = ufscustom_data.last() {
                writeln!(writer, "Time range: {:.3} - {:.3} ms", first.start_time, last.start_time)?;
            }
        }
        writeln!(writer)?;
    }
