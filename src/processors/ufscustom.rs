...
pub fn ufscustom_bottom_half_latency_process(mut ufscustom_list: Vec<UFSCUSTOM>) -> Vec<UFSCUSTOM> {
    ...
    println!(
        "Starting UFSCUSTOM bottom half latency processing for {} items",
        ufscustom_list.len()
    );
    ...
    for (i, ufscustom) in ufscustom_list.iter_mut().enumerate() {
        ...
        if let Some((prev_lba, prev_size, prev_opcode)) = &prev_request {
            ...
            ufscustom.continuous =
                ufscustom.lba == *prev_lba + *prev_size as u64 && ufscustom.opcode == *prev_opcode;
        } else {
        ...
        if let Some(prev_complete) = last_complete_time {
            ...
            ufscustom.ctoc = if time_diff >= 0.0 {
                time_diff * MILLISECONDS as f64
            } else {
                0.0
            };
        } else {
        ...
        if ufscustom.start_qd == 0 {
            if let Some(prev_qd_zero_complete) = last_qd_zero_complete_time {
                ...
                ufscustom.ctod = if time_diff >= 0.0 {
                    time_diff * MILLISECONDS as f64
                } else {
                    0.0
                };
    ...
    println!(
        "UFSCUSTOM bottom half processing completed in {:.2} seconds",
        elapsed.as_secs_f64()
    );
