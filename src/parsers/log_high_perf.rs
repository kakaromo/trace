...
pub fn parse_log_file_high_perf(
    filepath: &str,
    ...
    let chunk_size = if let Some(sys_info) = MemoryMonitor::get_system_memory_info() {
        ...
        println!(
            "Using adaptive chunk size: {:.2} MB",
    ...
    println!(
        "Using {} CPU cores with chunk size: {:.2} MB",
        cpu_count,
        final_chunk_size as f64 / 1_048_576.0
    );
...
pub fn parse_log_file_streaming(
    filepath: &str,
) -> io::Result<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> {
    let start_time = Instant::now();
    ...
    println!(
        "Using streaming buffer size: {:.2} MB",
        buffer_size as f64 / (1024.0 * 1024.0)
    );

    let cpu_count = num_cpus::get();
    println!("Using {cpu_count} CPU cores for parallel processing");
