...
// 비동기 로그 파싱 함수 (메인)
pub async fn parse_log_file_async(filepath: &str) -> io::Result<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> {
...
async fn parse_small_file_async(filepath: &str) -> io::Result<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> {
    let start_time = Instant::now();
    let (mut ufs_traces, mut block_traces, mut ufscustom_traces) = (Vec::new(), Vec::new(), Vec::new());
...
// 비동기 대용량 파일 처리 (스트리밍)
async fn parse_large_file_async(filepath: &str) -> io::Result<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> {
    let start_time = Instant::now();
    ...
    // 작업 채널들 생성
    let (result_sender, mut result_receiver) = mpsc::channel::<(usize, usize, usize)>(worker_count * 2);
            ...
            while let Some(chunk) = rx.recv().await {
                for line in &chunk {
                    if let Some((maybe_ufs, maybe_block, maybe_ufscustom)) = process_line(line) {
                        // UFS 처리
                        if let Some(ufs) = maybe_ufs {
                            if bincode::encode_into_std_write(&ufs, &mut ufs_writer, bincode::config::standard()).is_ok() {
                                local_ufs_count += 1;
                            }
                        }
                        ...
                        // Block 처리
                        if let Some(block) = maybe_block {
                            if bincode::encode_into_std_write(&block, &mut block_writer, bincode::config::standard()).is_ok() {
                                local_block_count += 1;
                            }
                        }
                        ...
                        // UFSCUSTOM 처리
                        if let Some(ufscustom) = maybe_ufscustom {
                            if bincode::encode_into_std_write(&ufscustom, &mut ufscustom_writer, bincode::config::standard()).is_ok() {
                ...
                // 결과 보고
                let _ = worker_sender.send((local_ufs_count, local_block_count, local_ufscustom_count)).await;
    ...
    // 각 데이터 타입 벡터 초기화 (초기화 값은 곧 덮어씌워질 것이므로 mut 사용 표시)
    let ufs_traces;
    let block_traces;
            ...
            while !reader.is_empty() {
                match bincode::decode_from_slice::<UFS, _>(&reader, config) {
                    Ok((ufs, size)) => {
                        ...
                        // 진행 상황 표시 (100만 항목마다)
                        if traces.len() % 1_000_000 == 0 && traces.len() > 0 {
            ...
            while !reader.is_empty() {
                match bincode::decode_from_slice::<UFSCUSTOM, _>(&reader, config) {
                    Ok((ufscustom, size)) => {
                        ...
                        // 진행 상황 표시 (10만 항목마다)
                        if traces.len() % 100_000 == 0 && traces.len() > 0 {
    ...
    // 비동기 작업 완료 대기
    ufs_traces = ufs_load.await?;
    block_traces = block_load.await?;
    ufscustom_traces = ufscustom_load.await?;
...
pub async fn parse_ufscustom_file_async(filepath: &str) -> io::Result<Vec<UFSCUSTOM>> {
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        if chunk.len() >= CHUNK_SIZE {
            ...
            if ufscustom_traces.len() % 1_000_000 == 0 {
                println!("UFSCustom 항목 {}백만 개 처리됨", ufscustom_traces.len() / 1_000_000);
            }
        }
    }
    ...
    let sorted = task::spawn_blocking(move || {
        ...
        traces.par_sort_by(|a, b| a.dtoc.partial_cmp(&b.dtoc).unwrap_or(std::cmp::Ordering::Equal));
        traces
    }).await?;
