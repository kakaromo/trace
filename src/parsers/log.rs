...
fn process_chunk_streaming(
    ...
    for line in chunk {
        ...
        // UFSCUSTOM 패턴 매칭
            ...
            // Serialize to Bincode format
            bincode::encode_into_std_write(&ufscustom, &mut *ufscustom_writer, bincode::config::standard())
...
fn process_chunk_parallel(chunks: Vec<Vec<String>>) -> (Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>) {
    ...
    let results: Vec<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> = chunks
        .into_par_iter()
        .map(process_lines)
        .collect();
...
fn parse_log_file_in_memory(filepath: &str) -> io::Result<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> {
    ...
    if let Ok(mmap) = unsafe { MmapOptions::new().map(&file) } {
        ...
        let chunks: Vec<_> = lines
            ...
            .map(|chunk| chunk.iter().map(|&s| s.to_string()).collect::<Vec<String>>())
            .collect();
...
fn parse_log_file_streaming(filepath: &str) -> io::Result<(Vec<UFS>, Vec<Block>, Vec<UFSCUSTOM>)> {
    ...
    println!("파일 크기: {}MB, 총 {}개 청크로 처리", file_size / 1_048_576, total_chunks);
    ...
    if let Ok(mmap) = unsafe { MmapOptions::new().map(&file) } {
        ...
        // 청크별로 병렬 처리
        (0..total_chunks).into_par_iter().for_each_with(sender.clone(), |s, chunk_idx| {
        ...
        for (_, lines) in receiver.iter().collect::<Vec<_>>().into_iter() {
            ...
            let (ufs_count, block_count, ufscustom_count) =
                process_chunk_streaming(&lines, &mut ufs_writer, &mut block_writer, &mut ufscustom_writer);
    ...
    // UFS 데이터 로드 (메모리 매핑 사용)
    {
        ...
        if let Ok(mmap) = unsafe { MmapOptions::new().map(&file) } {
            println!("UFS 데이터를 위한 메모리 매핑 성공 ({} MB)", file_size / 1_048_576);
            ...
            let _content = match std::str::from_utf8(&mmap[..]) {
                ...
                Err(e) => {
                    println!("메모리 매핑된 파일을 UTF-8로 변환할 수 없음: {}", e);
                    return Err(io::Error::new(io::ErrorKind::InvalidData, "UTF-8 변환 실패"));
                }
            };
            ...
            while !reader.is_empty() {
                match bincode::decode_from_slice::<UFS, _>(&reader, config) {
                    Ok((ufs, size)) => {
                        ...
                        // 진행 상황 표시 (100만 항목마다)
                        if ufs_traces.len() % 1_000_000 == 0 && ufs_traces.len() > 0 {
    ...
    // Block 데이터 로드 (메모리 매핑 사용)
    {
        ...
        if let Ok(mmap) = unsafe { MmapOptions::new().map(&file) } {
            println!("Block 데이터를 위한 메모리 매핑 성공 ({} MB)", file_size / 1_048_576);
            ...
            while !reader.is_empty() {
                match bincode::decode_from_slice::<Block, _>(&reader, config) {
                    Ok((block, size)) => {
                        ...
                        // 진행 상황 표시 (100만 항목마다)
                        if block_traces.len() % 1_000_000 == 0 && block_traces.len() > 0 {
                            println!("Block 항목 {} 백만 개 로드됨", block_traces.len() / 1_000_000);
                        }
                    }
                    Err(e) => {
                        // 역직렬화 오류 발생 시 남은 데이터 건너뛰기
    ...
    // UFSCUSTOM 데이터 로드 (메모리 매핑 사용)
    {
        ...
        if let Ok(mmap) = unsafe { MmapOptions::new().map(&file) } {
            println!("UFSCUSTOM 데이터를 위한 메모리 매핑 성공 ({} MB)", file_size / 1_048_576);
            ...
            while !reader.is_empty() {
                match bincode::decode_from_slice::<UFSCUSTOM, _>(&reader, config) {
                    Ok((ufscustom, size)) => {
                        ...
                        // 진행 상황 표시 (10만 항목마다)
                        if ufscustom_traces.len() % 100_000 == 0 && ufscustom_traces.len() > 0 {
                            println!("UFSCUSTOM 항목 {} 백만 개 로드됨", ufscustom_traces.len() / 1_000_000);
                        }
                    }
                    Err(e) => {
                        // 역직렬화 오류 발생 시 남은 데이터 건너뛰기
...
fn process_ufscustom_content(content: &str) -> io::Result<Vec<UFSCUSTOM>> {
    ...
    ufscustom_traces.par_sort_by(|a, b| a.dtoc.partial_cmp(&b.dtoc).unwrap_or(std::cmp::Ordering::Equal));
    ...
    if !ufscustom_traces.is_empty() {
        ...
        // 병렬 처리로 평균 계산
        let sum_dtoc = ufscustom_traces.par_iter()
            .map(|u| u.dtoc)
            .sum::<f64>();
