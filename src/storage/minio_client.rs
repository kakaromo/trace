...
impl MinioClient {
    ...
    pub fn new(config: &MinioConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let bucket = Bucket::new(&config.bucket, region, credentials)?.with_path_style(); // MinIO는 path-style 사용
    ...
    pub fn download_file(
        ...
        let response = self
            .runtime
            .block_on(async { self.bucket.get_object(remote_path).await })?;
    ...
    pub fn upload_file(
        ...
        let response = self
            .runtime
            .block_on(async { self.bucket.put_object(remote_path, &buffer).await })?;

        println!(
            "Upload completed: {} (status code: {})",
            remote_path,
    ...
    pub fn file_exists(&self, remote_path: &str) -> Result<bool, Box<dyn std::error::Error>> {
        let result = self
            .runtime
            .block_on(async { self.bucket.head_object(remote_path).await });
    ...
    pub fn delete_file(&self, remote_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.runtime
            .block_on(async { self.bucket.delete_object(remote_path).await })?;
    ...
    pub fn list_files(&self, prefix: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        let results = self
            .runtime
            .block_on(async { self.bucket.list(prefix.to_string(), None).await })?;
...
mod tests {
    ...
    fn test_minio_operations() {
        ...
        let access_key =
            env::var("MINIO_ACCESS_KEY").unwrap_or_else(|_| "test_access_key".to_string());
        let secret_key =
            env::var("MINIO_SECRET_KEY").unwrap_or_else(|_| "test_secret_key".to_string());
        let bucket = env::var("MINIO_BUCKET").unwrap_or_else(|_| "test_bucket".to_string());
