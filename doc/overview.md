...
### 자동 처리 모드 선택
- **기존**: `--highperf` 옵션으로 수동 모드 선택
- **변경**: 파일 크기 기반 자동 모드 선택 (1GB 기준)
- **장점**: 사용자가 파일 크기를 고려할 필요 없음
- **수동 제어**: `--streaming` 옵션으로 강제 스트리밍 모드 가능

### Block Parquet 스키마 개선
- **문제**: Block 모델과 Parquet 스키마 불일치
- **해결**: `models/block.rs` 정의와 완전히 일치하는 Parquet 스키마
- **추가**: `comm`, `qd` 필드 포함, `sector` 필드 정확한 매핑
